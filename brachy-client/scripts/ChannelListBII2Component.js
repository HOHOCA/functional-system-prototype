/**
 * 通道列表组件-B-II-2型 - 后装治疗计划系统
 * 功能：
 * - 显示所有通道信息（序号、名称、通道、驻留步长、出源长度、偏移）
 * - 动态数值列显示（红点表示激活，灰线表示非激活）
 * - 通道选择、编辑、删除
 * - 底部操作栏（选择使用孔洞、编辑、删除、删除所有）
 */
class ChannelListBII2Component {
    ...
    init() {
        if (!this.container) {
            console.error('ChannelListBII2Component: 容器不存在', this.containerId);
            return;
        }
        ...
        this.container.innerHTML = `
            <div class="channel-list-component">
                ...
                <div class="channel-list-toolbar">
                    <button class="channel-toolbar-btn" id="${this.options.prefix}selectHolesBtn" title="选择使用孔洞 添加模拟插植针或管道类施源器">
                        <i class="fas fa-plus"></i>
                    </button>
                    <button class="channel-toolbar-btn" id="${this.options.prefix}editBtn" title="编辑 编辑模拟插植针/管道类施源器" disabled>
    ...
    bindEvents() {
        // 选择使用孔洞
        const selectHolesBtn = document.getElementById(`${this.options.prefix}selectHolesBtn`);
        if (selectHolesBtn) {
            selectHolesBtn.addEventListener('click', () => {
                this.openSelectHolesDialog();
            });
        }
    ...
    openSelectHolesDialog() {
        // 打开B-II-2定位器选择使用孔洞弹窗
        this.showSelectHolesModal();
    }
    ...
    // 显示B-II-2定位器选择使用孔洞弹窗
    showSelectHolesModal() {
        // 如果已经存在弹窗，先关闭
        const existingModal = document.getElementById(`${this.options.prefix}selectHolesModal`);
        if (existingModal) {
            existingModal.remove();
        }
        ...
        // 创建弹窗容器
        const modal = document.createElement('div');
        modal.className = 'select-holes-modal';
        modal.id = `${this.options.prefix}selectHolesModal`;
        
        // 注入样式（如果还没有注入）
        this.injectSelectHolesModalStyles();

        // 弹窗内容HTML
        modal.innerHTML = this.getSelectHolesModalHTML();
        
        document.body.appendChild(modal);

        // 绑定事件
        this.bindSelectHolesModalEvents(modal);

        // 初始化弹窗数据
        this.initSelectHolesModalData(modal);
    }

    // 获取选择使用孔洞弹窗的HTML
    getSelectHolesModalHTML() {
        const prefix = this.options.prefix;
        return `
            <div class="select-holes-modal-header">
                <div class="select-holes-modal-title">选择使用孔洞</div>
                <button class="select-holes-modal-close" title="关闭">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="select-holes-modal-body">
                ...
                <div class="select-holes-controls">
                    ...
                    <div class="select-holes-oblique-container">
                        ${this.getObliqueHolesLeftHTML(prefix)}
                ...
                <div class="select-holes-modal-footer">
                    <button class="select-holes-btn select-holes-btn-cancel" id="${prefix}selectHolesCancelBtn">取消</button>
                    <button class="select-holes-btn select-holes-btn-confirm" id="${prefix}selectHolesConfirmBtn">确定</button>
                </div>
            </div>
        `;
    }
    ...
    // 获取外周孔HTML
    getPeripheralHolesHTML(prefix) {
        const cHoles = Array.from({length: 12}, (_, i) => i + 1);
        return `
            <div class="select-holes-control-group">
                <div class="select-holes-control-header">
                    <label class="select-holes-control-label">外周孔</label>
                    <select class="select-holes-select" id="${prefix}peripheralHolesSelect">
                        <option value="not-use">不使用</option>
                        <option value="needle1">插植针1</option>
                        <option value="needle2">插植针2</option>
                    </select>
                </div>
                <div class="select-holes-checkbox-group" id="${prefix}peripheralHolesCheckboxes">
                    ${cHoles.map(num => `
                        <label class="select-holes-checkbox-label">
                            <input type="checkbox" class="select-holes-checkbox" 
                                   data-hole="C${num}" 
                                   id="${prefix}peripheralC${num}">
                            <span>C${num}</span>
                        </label>
                    `).join('')}
                </div>
    ...
    // 获取中间孔HTML
    getMiddleHoleHTML(prefix) {
        return `
            <div class="select-holes-control-group">
                <div class="select-holes-control-header">
                    <label class="select-holes-control-label">中间孔C13</label>
                    <select class="select-holes-select" id="${prefix}middleHoleSelect">
                        <option value="not-use">不使用</option>
                        <option value="tube1">管状施源器1</option>
                        <option value="tube2">管状施源器2</option>
                    </select>
                </div>
            </div>
        `;
    ...
    // 获取斜针孔（左）HTML
    getObliqueHolesLeftHTML(prefix) {
        const holes = ['A/B1', 'A/B2', 'A/B3', 'A/B4'];
        return `
            <div class="select-holes-control-group">
                <div class="select-holes-control-header">
                    <label class="select-holes-control-label">斜针孔（左）</label>
                    <select class="select-holes-select" id="${prefix}obliqueLeftSelect">
                        <option value="not-use">不使用</option>
                        <option value="needle1">插植针1</option>
                        <option value="needle2">插植针2</option>
                    </select>
                </div>
                <div class="select-holes-oblique-group" id="${prefix}obliqueLeftCheckboxes">
                    ${holes.map(hole => `
                        <div class="select-holes-oblique-item">
                            <label class="select-holes-checkbox-label">
                                <input type="checkbox" class="select-holes-checkbox" 
                                       data-hole="${hole}" 
                                       data-side="left"
                                       id="${prefix}obliqueLeft${hole}"
                                       disabled>
                                <span>${hole}</span>
                            </label>
                            <select class="select-holes-oblique-select" 
                                    id="${prefix}obliqueLeft${hole}Select" 
                                    disabled>
    ...
    // 获取斜针孔（右）HTML
    getObliqueHolesRightHTML(prefix) {
        const holes = ['A/B1', 'A/B2', 'A/B3', 'A/B4'];
        return `
            <div class="select-holes-control-group">
                <div class="select-holes-control-header">
                    <label class="select-holes-control-label">斜针孔（右）</label>
                    <select class="select-holes-select" id="${prefix}obliqueRightSelect">
                        <option value="not-use">不使用</option>
                        <option value="needle1">插植针1</option>
                        <option value="needle2">插植针2</option>
                    </select>
                </div>
                <div class="select-holes-oblique-group" id="${prefix}obliqueRightCheckboxes">
                    ${holes.map(hole => `
                        <div class="select-holes-oblique-item">
                            <label class="select-holes-checkbox-label">
                                <input type="checkbox" class="select-holes-checkbox" 
                                       data-hole="${hole}" 
                                       data-side="right"
                                       id="${prefix}obliqueRight${hole}"
                                       disabled>
                                <span>${hole}</span>
                            </label>
                            <select class="select-holes-oblique-select" 
                                    id="${prefix}obliqueRight${hole}Select" 
                                    disabled>
                                <option value="">(空)</option>
                            </select>
                        </div>
    ...
    getTipImageSVG() {
        ...
        const centerX = 200;
        const centerY = 150;
        const c13Radius = 15; // C13中心圆半径（比其他孔洞大）
        const cHolesRadius = 80; // C1-C12环形半径
        ...
        for (let i = 0; i < 4; i++) {
            const angle = (leftAStartAngle + (i * leftAAngleRange / 3)) * Math.PI / 180;
        ...
        for (let i = 0; i < 4; i++) {
            const angle = (rightAStartAngle - (i * leftAAngleRange / 3)) * Math.PI / 180;
            const radius = 140; // A系列半径
            const x = centerX + radius * Math.cos(angle);
            const y = centerY + radius * Math.sin(angle);
            // 序号反转：4-i (i=0->4, i=1->3, i=2->2, i=3->1)
        ...
        return `
            <svg viewBox="0 0 400 300" style="width: 100%; height: 100%;" id="${prefix}tipImageSVG">
                <!-- C13中心圆 -->
                <circle cx="${centerX}" cy="${centerY}" r="${c13Radius}" 
                        fill="white" stroke="#666" stroke-width="1"
                        class="hole-element" data-hole="C13" data-type="middle"/>
                <text x="${centerX}" y="${centerY + 4}" text-anchor="middle" 
                      fill="black" font-size="12" font-weight="bold">C13</text>
                
                <!-- C1-C12环形孔 -->
                ${cHoles.map(c => `
                    <circle cx="${c.x}" cy="${c.y}" r="${holeRadius}" 
                            fill="white" stroke="#666" stroke-width="1"
                            class="hole-element" data-hole="C${c.num}" data-type="peripheral"/>
                    <text x="${c.x}" y="${c.y + 4}" text-anchor="middle" 
                          fill="black" font-size="10">C${c.num}</text>
                `).join('')}
                
                <!-- 左侧A1-A4 -->
                ${leftA.map(a => `
                    <circle cx="${a.x}" cy="${a.y}" r="${holeRadius}" 
                            fill="white" stroke="#666" stroke-width="1"
                            class="hole-element" data-hole="A${a.num}" data-side="left" data-type="oblique"/>
                    <text x="${a.x}" y="${a.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">A${a.num}</text>
                `).join('')}
                
                <!-- 左侧B1-B5 -->
                ${leftB.map(b => `
                    <circle cx="${b.x}" cy="${b.y}" r="${holeRadius}" 
                            fill="white" stroke="#666" stroke-width="1"
                            class="hole-element" data-hole="B${b.num}" data-side="left" data-type="oblique"/>
                    <text x="${b.x}" y="${b.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">B${b.num}</text>
                `).join('')}
                ...
                ${rightA.map(a => `
                    <circle cx="${a.x}" cy="${a.y}" r="${holeRadius}" 
                            fill="white" stroke="#666" stroke-width="1"
                            class="hole-element" data-hole="A${a.num}" data-side="right" data-type="oblique"/>
                    <text x="${a.x}" y="${a.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">A${a.num}</text>
                `).join('')}
                ...
                ${rightB.map(b => `
                    <circle cx="${b.x}" cy="${b.y}" r="${holeRadius}" 
                            fill="white" stroke="#666" stroke-width="1"
                            class="hole-element" data-hole="B${b.num}" data-side="right" data-type="oblique"/>
                    <text x="${b.x}" y="${b.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">B${b.num}</text>
                `).join('')}
            </svg>
    ...
    getTailImageSVG() {
        ...
        const centerY = 150;
        const c13Radius = 15; // C13中心圆半径（比其他孔洞大）
        const cHolesRadius = 80;
        const holeRadius = 10; // 其他所有孔洞的统一半径（C1-C12）
        const rectWidth = 25; // A/B矩形宽度
        ...
        return `
            <svg viewBox="0 0 400 300" style="width: 100%; height: 100%;" id="${prefix}tailImageSVG">
                <!-- C13中心圆 -->
                <circle cx="${centerX}" cy="${centerY}" r="${c13Radius}" 
                        fill="white" stroke="#666" stroke-width="1"
                        class="hole-element" data-hole="C13" data-type="middle"/>
                ...
                <!-- 左侧A/B1-A/B4矩形 -->
                ${leftAB.map(ab => `
                    <rect x="${ab.x - rectWidth/2}" y="${ab.y - rectHeight/2}" 
                          width="${rectWidth}" height="${rectHeight}" 
                          fill="white" stroke="#666" stroke-width="1"
                          class="hole-element" data-hole="A/B${ab.num}" data-side="left" data-type="oblique"/>
                    <text x="${ab.x}" y="${ab.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">A/B${ab.num}</text>
                `).join('')}
                ...
                ${rightAB.map(ab => `
                    <rect x="${ab.x - rectWidth/2}" y="${ab.y - rectHeight/2}" 
                          width="${rectWidth}" height="${rectHeight}" 
                          fill="white" stroke="#666" stroke-width="1"
                          class="hole-element" data-hole="A/B${ab.num}" data-side="right" data-type="oblique"/>
                    <text x="${ab.x}" y="${ab.y + 4}" text-anchor="middle" 
                          fill="black" font-size="9">A/B${ab.num}</text>
                `).join('')}
            </svg>
    ...
    getSelectHolesModalStyles() {
        return `
            ...
            .select-holes-modal-title {
                font-size: 16px;
                font-weight: 500;
                color: #ddd;
            }

            .select-holes-modal-close {
                background: none;
                border: none;
                color: #888;
                font-size: 20px;
            ...
            .select-holes-modal-close:hover {
                background: #333;
                color: #ddd;
            }
            ...
            .select-holes-checkbox-label:has(input:disabled) {
                color: #666;
                cursor: not-allowed;
            }

            .select-holes-checkbox {
                cursor: pointer;
                width: 14px;
                height: 14px;
                flex-shrink: 0;
            ...
            .select-holes-checkbox:disabled {
                cursor: not-allowed;
            }
            ...
            .select-holes-oblique-item .select-holes-checkbox-label {
                flex: 0 0 60px;
            }

            .select-holes-oblique-select {
                flex: 1;
                padding: 5px 8px;
                background: #111;
                border: 1px solid #404040;
            ...
            .select-holes-oblique-select:disabled {
                background: #0a0a0a;
                color: #666;
                cursor: not-allowed;
            }
            ...
            .select-holes-btn-cancel {
                background: #2d2d2d;
                color: #ddd;
            }

            .select-holes-btn-cancel:hover {
                background: #373737;
            }

            .select-holes-btn-confirm {
                background: #3AACDE;
                color: #fff;
                border-color: #3AACDE;
            }

            .select-holes-btn-confirm:hover {
                background: #2d8bb8;
            }
        `;
    }
    ...
    // 绑定弹窗事件
    bindSelectHolesModalEvents(modal) {
        const prefix = this.options.prefix;

        // 关闭按钮
        const closeBtn = modal.querySelector('.select-holes-modal-close');
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                this.closeSelectHolesModal();
            });
        }

        // 取消按钮
        const cancelBtn = document.getElementById(`${prefix}selectHolesCancelBtn`);
        if (cancelBtn) {
        ...
        // 确定按钮
        const confirmBtn = document.getElementById(`${prefix}selectHolesConfirmBtn`);
        if (confirmBtn) {
            confirmBtn.addEventListener('click', () => {
                this.confirmSelectHoles(modal);
            });
        }

        // 外周孔下拉选择变化
        const peripheralSelect = document.getElementById(`${prefix}peripheralHolesSelect`);
        if (peripheralSelect) {
            peripheralSelect.addEventListener('change', () => {
                this.handlePeripheralSelectChange(modal);
            });
        }

        // 中间孔下拉选择变化
        const middleSelect = document.getElementById(`${prefix}middleHoleSelect`);
        if (middleSelect) {
            middleSelect.addEventListener('change', () => {
                this.handleMiddleSelectChange(modal);
            });
        }

        // 斜针孔左下拉选择变化
        const obliqueLeftSelect = document.getElementById(`${prefix}obliqueLeftSelect`);
        if (obliqueLeftSelect) {
            obliqueLeftSelect.addEventListener('change', () => {
                this.handleObliqueSelectChange(modal, 'left');
            });
        }

        // 斜针孔右下拉选择变化
        const obliqueRightSelect = document.getElementById(`${prefix}obliqueRightSelect`);
        if (obliqueRightSelect) {
            obliqueRightSelect.addEventListener('change', () => {
                this.handleObliqueSelectChange(modal, 'right');
            });
        }

        // 外周孔复选框变化
        const peripheralCheckboxes = modal.querySelectorAll(`#${prefix}peripheralHolesCheckboxes .select-holes-checkbox`);
        peripheralCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                this.handlePeripheralCheckboxChange(modal, checkbox);
            });
        });

        // 斜针孔复选框变化
        const obliqueCheckboxes = modal.querySelectorAll('.select-holes-oblique-group .select-holes-checkbox');
        obliqueCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                this.handleObliqueCheckboxChange(modal, checkbox);
            });
        });

        // 斜针孔下拉选择变化
        const obliqueSelects = modal.querySelectorAll('.select-holes-oblique-select');
        obliqueSelects.forEach(select => {
            select.addEventListener('change', () => {
                this.handleObliqueSelectSelectChange(modal, select);
            });
        });
    ...
    // 初始化弹窗数据
    initSelectHolesModalData(modal) {
        // TODO: 从当前计划加载数据
        // 这里先设置默认值，初始化时所有下拉框都是"不使用"，所以复选框应该都是禁用状态
        const prefix = this.options.prefix;
        
        // 初始化外周孔复选框状态
        const peripheralSelect = document.getElementById(`${prefix}peripheralHolesSelect`);
        if (peripheralSelect && peripheralSelect.value === 'not-use') {
            this.handlePeripheralSelectChange(modal);
        }
        
        // 初始化斜针孔（左）复选框状态
        const obliqueLeftSelect = document.getElementById(`${prefix}obliqueLeftSelect`);
        if (obliqueLeftSelect && obliqueLeftSelect.value === 'not-use') {
            this.handleObliqueSelectChange(modal, 'left');
        }
        
        // 初始化斜针孔（右）复选框状态
        const obliqueRightSelect = document.getElementById(`${prefix}obliqueRightSelect`);
        if (obliqueRightSelect && obliqueRightSelect.value === 'not-use') {
            this.handleObliqueSelectChange(modal, 'right');
        }
    }

    // 处理外周孔选择变化
    handlePeripheralSelectChange(modal) {
        const prefix = this.options.prefix;
        const select = document.getElementById(`${prefix}peripheralHolesSelect`);
        const checkboxes = modal.querySelectorAll(`#${prefix}peripheralHolesCheckboxes .select-holes-checkbox`);
        
        if (select.value === 'not-use') {
            checkboxes.forEach(cb => {
                cb.checked = false;
                cb.disabled = true;
                // 清除图像高亮
                const hole = cb.dataset.hole;
                this.updateImageHighlight(modal, hole, false);
            });
        } else {
            checkboxes.forEach(cb => {
                cb.disabled = false;
    ...
    // 处理中间孔选择变化
    handleMiddleSelectChange(modal) {
        const prefix = this.options.prefix;
        const select = document.getElementById(`${prefix}middleHoleSelect`);
        const isHighlighted = select.value !== 'not-use';
        this.updateImageHighlight(modal, 'C13', isHighlighted);
    }

    // 处理斜针孔选择变化
    handleObliqueSelectChange(modal, side) {
        const prefix = this.options.prefix;
        const select = document.getElementById(`${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Select`);
        const checkboxes = modal.querySelectorAll(`#${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Checkboxes .select-holes-checkbox`);
        const selects = modal.querySelectorAll(`#${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Checkboxes .select-holes-oblique-select`);
        ...
        if (select.value === 'not-use') {
            ...
            // 清除所有对应的尖端出口高亮
            this.clearObliqueTipHighlights(modal, side);
        } else {
            checkboxes.forEach(cb => {
                cb.disabled = false;
            });
            // 更新下拉选项
            this.updateObliqueSelectOptions(modal, side);
        }
    }

    // 处理外周孔复选框变化
    handlePeripheralCheckboxChange(modal, checkbox) {
        const hole = checkbox.dataset.hole;
        const isChecked = checkbox.checked;
        this.updateImageHighlight(modal, hole, isChecked);
    }
    
    // 更新图像高亮状态
    updateImageHighlight(modal, holeId, isHighlighted, side = null) {
        const prefix = this.options.prefix;
        const tipSVG = document.querySelector(`#${prefix}tipImageSVG`);
        const tailSVG = document.querySelector(`#${prefix}tailImageSVG`);
        
        // 构建选择器，如果有side参数则加上data-side条件
        const selector = side ? `[data-hole="${holeId}"][data-side="${side}"]` : `[data-hole="${holeId}"]`;
        
        // 查找对应的SVG元素
        const tipElement = tipSVG?.querySelector(selector);
        const tailElement = tailSVG?.querySelector(selector);
        
        if (tipElement) {
            tipElement.setAttribute('fill', isHighlighted ? '#3AACDE' : 'white');
        }
        if (tailElement) {
            tailElement.setAttribute('fill', isHighlighted ? '#3AACDE' : 'white');
        }
    }
    
    // 更新多个图像高亮状态
    updateImageHighlights(modal, holeIds, isHighlighted) {
        holeIds.forEach(holeId => {
            this.updateImageHighlight(modal, holeId, isHighlighted);
        });
    }

    // 处理斜针孔复选框变化
    handleObliqueCheckboxChange(modal, checkbox) {
        const prefix = this.options.prefix;
        const hole = checkbox.dataset.hole;
        const side = checkbox.dataset.side;
        const select = document.getElementById(`${prefix}oblique${side === 'left' ? 'Left' : 'Right'}${hole}Select`);
        
        // 更新尾端图像高亮（需要指定side）
        this.updateImageHighlight(modal, hole, checkbox.checked, side);
        
        if (checkbox.checked) {
            select.disabled = false;
            this.updateObliqueSelectOptions(modal, side);
        } else {
            // 先清除对应的尖端出口高亮（需要指定side）
            const tipHole = select.value;
            if (tipHole) {
                this.updateImageHighlight(modal, tipHole, false, side);
            }
    ...
    // 更新斜针孔下拉选项
    updateObliqueSelectOptions(modal, side) {
        // 根据需求：下拉栏中可选内容为其他斜针孔（左/右）没有选择的尖端出口序号
        const prefix = this.options.prefix;
        const availableOptions = ['A1', 'A2', 'A3', 'A4', 'B1', 'B2', 'B3', 'B4', 'B5'];
        const checkboxes = modal.querySelectorAll(`#${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Checkboxes .select-holes-checkbox:checked`);
        ...
        // 更新所有下拉框的选项
        const allSelects = modal.querySelectorAll(`#${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Checkboxes .select-holes-oblique-select`);
        allSelects.forEach(select => {
            if (!select.disabled) {
                const currentValue = select.value;
                select.innerHTML = '<option value="">(空)</option>';
                availableOptions.forEach(opt => {
                    if (!selectedOptions.has(opt) || opt === currentValue) {
                        const option = document.createElement('option');
                        option.value = opt;
                ...
                if (currentValue) {
                    ...
                    select.dataset.oldValue = currentValue;
                }
            }
        });
    }
    ...
    // 处理斜针孔下拉选择变化
    handleObliqueSelectSelectChange(modal, select) {
        const prefix = this.options.prefix;
        const side = select.id.includes('Left') ? 'left' : 'right';
        const holeMatch = select.id.match(/oblique(Left|Right)(A\/B\d+)Select/);
        
        if (holeMatch) {
            const hole = holeMatch[2];
            const oldValue = select.dataset.oldValue || '';
            const newValue = select.value;
            
            // 清除旧的高亮（尖端图像需要指定side）
            if (oldValue) {
                this.updateImageHighlight(modal, oldValue, false, side);
            }
            
            // 设置新的高亮（尖端图像需要指定side）
            if (newValue) {
                this.updateImageHighlight(modal, newValue, true, side);
            }
    ...
    // 清除斜针孔对应的尖端出口高亮
    clearObliqueTipHighlights(modal, side) {
        const prefix = this.options.prefix;
        const selects = modal.querySelectorAll(`#${prefix}oblique${side === 'left' ? 'Left' : 'Right'}Checkboxes .select-holes-oblique-select`);
        selects.forEach(select => {
            const value = select.value;
            if (value) {
                this.updateImageHighlight(modal, value, false, side);
            }
        });
    }

    // 确认选择
    confirmSelectHoles(modal) {
        // TODO: 实现确认逻辑，保存选择并更新通道列表
        console.log('确认选择使用孔洞');
        this.closeSelectHolesModal();
    }

    // 关闭弹窗
    closeSelectHolesModal() {
        const modal = document.getElementById(`${this.options.prefix}selectHolesModal`);
        if (modal) {
